...
typedef struct {
    ...
    WPHeizlast,WPHeizgrenze,WPLeistung,WPmin,WPmax,WPPVon,WPPVoff,WPEHZ,WPZWE,WPZWEPVon,WPHK1,WPHK1max,WPHK2on,WPHK2off,WPOffset,BWWPein,BWWPaus,BWWPon,BWWPoff, BWWPSupport,BWWPTasmotaDauer,ForcecastSoc,ForcecastConsumption,ForcecastReserve;
...
void mewp(std::vector<watt_s> &w,std::vector<wetter_s>&wetter,float &fatemp,float &cop,int sunrise, int sunset,e3dc_config_t &e3dc, float soc, int ireq_Heistab, float zuluft, float notromreserve,int32_t iHeatStat[24*4+2]);
