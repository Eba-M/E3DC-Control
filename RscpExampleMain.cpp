...
void WriteLiveJSON() {
    ...
    cJSON_AddNumberToObject(root, "Wallbox_Power", fPower_WB+fPower_openWB);
...
//                if(sscanf(line, "%[^ \t=]%*[\t ]=%*[\t ]%[^\n]", var, value) == 2) {
                  ...
                  else if((strcmp(var, "prognosetest") == 0))
                  {
                      if (strcmp(value, "false") == 0)
                          e3dc_config.prognosetest = false;
                      else
...
// Active Power Limit    61441
            ...
            if (e.size()>0&&dimm<100&&dimm>=0&&(e.begin()->pp>e3dc_config.DVmp*-10 || fPower_Grid >-200))
            {
                if (fPower_Grid>300)
                    dimm = dimm + fPower_Grid/300;
...
//    sprintf(buf,"E3DC-Control/Avl -m %i",iAvalPower);
    ...
    MQTTsend(e3dc_config.mqtt2_ip,buf);
    sprintf(buf,"E3DC-Control/Grid -m '%0.2f %0.2f %0.2f %0.2f %0.2f'",fPower_Grid,fBatt_SOC,float(iPower_Bat),fcurrentGrid,fsollGrid);
    MQTTsend(e3dc_config.mqtt2_ip,buf);
...
int LoadDataProcess() {
    ...
    if (t%10==0)
    {
        MQTTWB(fPower_openWB);
//        fPower_openWB = 3333.33;
    }
...
// Uhrzeit Average prog. Solar real Solar % letzte Progose Ertrag real Ertrag % kumm tagesverbrauch Haus WP
                    fprintf(fp,"%0.2f %0.2f%% %0.2f%% %0.2f / %0.2f%% %0.2f%% %0.2f / %0.2f %0.2f %0.2f° %0.2f %0.4f \n",f4,f2,f3,f3/f2,w_alt.progsolar,f5,f5/w_alt.progsolar,iWeekhour[dayhour]/3600000.0,iWeekhourWP[dayhour]/3600000.0,fatemp,ftemp[0],iGridStat[Gridstat1]/900000.0);
                    fclose(fp);
                }
            }
...
//                    bHK1off = 0;
                ...
                (
                    ...
                    || (temp[1]>=1 && temp[15]+20>temp[4]) // Wenn die Puffertemp noch ausreichend hoch
...
//                            (wolf[wpvl].wert>0&&wolf[wpvl].wert*10>temp[10]+50)
                         ...
                         // Puffertemperaturen zu hoch ??
                            (temp[15]>(e3dc_config.WPHK1max+3)*10&&wolf[wpvl].wert>(e3dc_config.WPHK1max+4.0)
                             &&
                             wolf[wpvl].wert>0)
                         ||
                         ...
                         (temp[7]>0&&temp[12]>0&&temp[10]<(wolf[wpvl].wert-1)*10&&temp[11]>=temp[10]+5
                          &&
                          temp[1]>0&&wolf[wpvl].wert>0&&(wolf[wpvl].wert-6)*10>temp[4])
                         ||
...
//                                ALV = 0;
                        ...
                        if (
                            ...
                            && (temp[1]==0||(wolf[wprl].wert==0||(wolf[wprl].wert*10<temp[4]+10||temp[5]<350||temp[4]>temp[5])))
                            &&
                            ...
                            (
                             ...
                             ||
                             // RL < FBH Soll + 2° nur bei Überschuss
                             (temp[7]>0&&temp[15]<=(temp[4]+20)&&PVon>e3dc_config.WPPVon)
                             // Soll >== Ist+10
                             ||
...
// PVon dynamischer Berechnung unter Ausnutzung des Rest SoC am Morgen
    ...
    if (iMinlade>iMaxBattLade) iMinlade = iMaxBattLade;
        if (t<tLadezeitende3&&(t/60)>sunriseAt&&fPVtoday>fPVSoll&&fBatt_SOC>5&&iPower_PV>100)
        {
            ...
            PVon = PVon*.9 + (fdynPower+((-iMinlade/2 + iPower_PV - iPowerHome - fPower_Grid+fPower_WB)))/10;
        }
        else
...
// Es wird nachts nur noch bis e3dc_config.peakshaveuppersoc aus dem Netz geladen
                            if ((fpeakshaveendsoc<fBatt_SOC&&e3dc_config.peakshaveuppersoc<fBatt_SOC)&&iPower_PV_E3DC==0&&iFc>0)
                                iFc = -2000;
...
// Das Nachladen geschieht nur wenn Netzbezug besteht und auch der Master nachlädt
                                    {
                                        ...
                                        if (iBattLoad - fcurrentGrid + fsollGrid - 50<iFc*2)
...
// Wenn der eigene SoC 5% über dem Master SoC liegt, dann wird nicht geladen
            ...
            } else
            {
                ...
                if (e3dc_config.debug1)
                    printf("#  3 %5.2f %2i",fpeakshaveminsoc,iFc);
                printf("%c[K\n",27);
                printf("shavingB = %li ",t-itime);
//                printf("%5.2f ",fpeakshaveminsoc);
                printf("%5.2f %5i %5.2f %5.2f%c[K\n",fpeakshaveminsoc,iFc,fPower_Ext[2],fPower_Ext[3],27);
//                printf("shavingB = %li %5.2f %5i %5.2f %5.2f%c[K\n",t-itime,fpeakshaveminsoc,iFc,fPower_Ext[2],fPower_Ext[3],27);
    ...
    if (wetter.size()>0)
    printf("%.2f° %.2f° %.2f° %.2f°",wetter[0].temp,fatemp24,fatemp,ftemp[0]);
    printf("%c[K\n", 27 );
        ...
        if ((e3dc_config.wbmode>=0)) // Dose verriegelt, bereit zum Laden
        {
            ...
            switch (e3dc_config.wbmode)
            {
                ...
                case 10:
                    ...
                    {
                        if (iPower>0)
                            iPower = iPower + e3dc_config.wbminlade*-1;
                        else
                ...
                switch(PMData[i].tag) {
                    ...
                    case TAG_PM_POWER_L3: {              // response for TAG_PM_REQ_L3
                        ...
                        printf("#%0.1fW ", fPower1+fPower2+fPower3);
                            if (fPower_openWB>=0)
                                printf("openWB %0.1fW",fPower_openWB);
                        printf("%c[K\n", 27 );
...
//                printf("Request data done %s %2ld:%2ld:%2ld",VERSION,tm_CONF_dt%(24*3600)/3600,tm_CONF_dt%3600/60,tm_CONF_dt%60);
                printf("%s %2ld:%2ld:%2ld %2li %3li",VERSION,tm_CONF_dt%(24*3600)/3600,tm_CONF_dt%3600/60,tm_CONF_dt%60,(t_alt-myclock)%100,1000ms-ms);
...
//    int ret = solaredge();
    ...
    if (e3dc_config.statistik)
    {
        ...
        while(iEC < 10&&e3dc_config.stop==0||e3dc_config.stop>99)
        {
            ...
            printf(e3dc_config.logfile);            WriteLog(e3dc_config,Log1,0);
            // connect to server
