...
//                    return 0;
        ...
        if ((fPower_Grid>200)&&(fAvPower_Grid > 100)&&(iPower_Bat > -200)&&(fAvBatterie>=-200)&&fBatt_SOC>fNotstromreserve&&idauer==0)  // es wird Strom bezogen Entladesperre solange aufheben
        {
    ...
    if ((e3dc_config.wbmode>0)) // Dose verriegelt, bereit zum Laden
    {
        ...
        switch (e3dc_config.wbmode)
        {
            case 1:
                ...
                iPower = -fPower_Grid-e3dc_config.einspeiselimit*1000+500+iWBMinimumPower; // Schon 500W früher einschalten
                //                iPower = -fPower_Grid-e3dc_config.einspeiselimit*1000;
                if (fPower_WB > 1000)
                    //                    iPower = iPower+iPower_Bat-iRefload+iWBMinimumPower/6;
                    iPower = -fPower_Grid-e3dc_config.einspeiselimit*1000+1000+iWBMinimumPower-fPower_WB; // Schon 500W früher
...
// Grid
                    if (e3dc_config.WP)
                    {
                        ...
                        int x4 = (t+900)%(24*3600)/900+2;
                        printf(" WB %0.02f %0.02f %0.02f %0.02f %0.02f",waermebedarf,float(iHeatStat[1]/3600000.0),float(iHeatStat[x4]/900000.0),float(iHeatStat[x2]/900000.0),float(iHeatStat[0])/(t%900)/1000);
                    }
                }
                printf("%c[K\n", 27 );
