          ...
          }
          {
            ...
            int x2 = 0;
              ...
              if  (item1!=NULL)
              {
                  ...
                  while (w->size()>0&&w->begin()->hh < item1->valueint)
                      //                    x1++;
                      w->erase(w->begin());
                  while (wetter->size()>0&&wetter->begin()->hh < item1->valueint)  // um 15min verschieben
...
//                    if (wetter->at(x2).hh == item1->valueint-900)
                {
                    wetter->at(x2).temp = item3->valuedouble;
...
void forecast(std::vector<wetter_s> &w, e3dc_config_t e3dc_config,int anlage)
...
// die Wetterdaten alle 15min + 30sekundenin  holen,
        ...
        )
    {
        ...
        //        system("curl -X GET 'https://api.openweathermap.org/data/3.0/onecall?lat=50.2525&lon=10.3083&appid=615b8016556d12f6b2f1ed40f5ab0eee' | jq .hourly| jq '.[]' | jq '.dt%259200/3600, .clouds'>weather.out");
        ...
        {
            if (strlen(e3dc.Forecast[0])==0)
            {
                strcpy(e3dc.Forecast[0],"0/0/10");
                forecast (wetter, e3dc, 0);
            }
            else
                for (int j=0;(strlen(e3dc.Forecast[j])>0)&&j<4;j++)
                    forecast(wetter, e3dc, j);
        }
    }
...
//alle alten einträge löschen ch = 1 ändernung 9.8.25 die alten einträge bleiben gültig
    if (ch.size()>0)
    {
        ...
        {
            while (ch.size()>j&&ch[j].ch == chch)
                if (ch[j].hh+900 < rawtime||ladedauer == 0)
                ch.erase(ch.begin()+j);
                else break;
